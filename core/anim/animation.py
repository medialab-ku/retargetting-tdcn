...
import core.anim.animation_structure as astruct
...
class Animation:
    ...
    def __init__(self, rotations, positions, orients, offsets, parents, eulers):
    ...
    def __iop__(self, op, other):
        ...
        self.eulers = op(self.eulers, other.eulers)
        return self
    ...
    def __getitem__(self, k):
        if isinstance(k, tuple):
            raise Exception("I dont know what exactly mean")
            return Animation(
                self.rotations[k],
                self.positions[k],
                self.orients[k[1:]],
    ...
    def __setitem__(self, k, v):
        if isinstance(k, tuple):
            ...
            self.eulers.__setitem__(k, v.eulers)
        else:
            self.rotations.__setitem__(k, v.rotations)
            self.positions.__setitem__(k, v.positions)
    ...
    def copy(self):
        return Animation(
            ...
            self.parents.copy(), self.eulers.copy())

    def repeat(self, *args, **kw):
        return Animation(
            self.rotations.repeat(*args, **kw),
            self.positions.repeat(*args, **kw),
            self.orients, self.offsets, self.parents, self.eulers.repeat(*args, **kw))

    def ravel(self):
        raise Exception("I dont know what exactly do something")
        # return np.hstack([
...
def transforms_global(anim, ignore_root=False):
    """
...
def positions_local(anim):
    ...
    positions = transforms_global(anim, ignore_root=True)[:, :, :, 3]
